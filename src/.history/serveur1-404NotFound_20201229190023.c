    ...
    while (1)
    {
        ...
        if (fork() == 0)
        {
            ...
            struct timeval old_timeout;
                ...
                old_timeout.tv_sec = 2 * (t2.tv_sec - t1.tv_sec);
                old_timeout.tv_usec = 2 * (t2.tv_usec - t1.tv_usec);
            ...
            while (ack_obtenu != times)
            {
                ...
                else if (ret == 0)
                {
                    ...
                    seuil = window_size / 2;
                    window_size = 2;
                    old_timeout.tv_sec = old_timeout.tv_sec * 2;
                    old_timeout.tv_usec = old_timeout.tv_usec * 2; 
                    goto RETRANSMISSION;
                }
                ...
                if (FD_ISSET(msg_socket, &fd))
                {
                    ...
                    timeout.tv_sec = 0.7*old_timeout.tv_sec+0.3*t_ecart.tv_sec;
                    timeout.tv_usec = 0.7*old_timeout.tv_usec+0.3*t_ecart.tv_usec;
                    printf("timeout = %d", timeout.tv_usec);
                }
