...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        if (fork() == 0)
        {
            ...
            struct timeval table_timeout[window_size];
            ...
            struct timeval t_commence, t_finit, t_ecart;
            ...
            while (ack_obtenu != times)
            {
                ...
                if (last_ack == ack_obtenu && repeat_time == 0)
                {
                    window_size = 2;
                RETRANSMISSION:
                    for (i = window_head; i < window_head + window_size + 1; i++)
                    {
                        bzero(buffer_msg, RCVSIZE);
                        ...
                        gettimeofday(&time_of_packet, NULL);
                        table_timeout[i - window_head - 1] = time_of_packet;
                ...
                else if (last_ack < ack_obtenu)
                {
                    ...
                    {
                        table_timeout[i] = table_timeout[(i + ack_obtenu - last_ack) % (window_size - 1)];
                    }
                ...
                else if (ret == 0)
                {
                    ...
                    window_size = 2;
                    timeout_times++;
                    timeout.tv_sec = old_timeout.tv_sec * (1 + 0.5 / timeout_times);
                    timeout.tv_usec = old_timeout.tv_usec * (1 + 0.5 / timeout_times);
                    printf("timeout = %d\n\n", timeout.tv_usec);
                ...
                if (FD_ISSET(msg_socket, &fd))
                {
                    ...
                    gettimeofday(&t_finit, NULL);
                    printf("timeout_finit = %d\n", t_finit.tv_usec);
                }
                ...
                else if (ack_obtenu > last_ack)
                {
                    printf("timeout_n = %d\n", table_timeout[0].tv_usec);
                                        printf("timeout_n = %d\n", table_timeout[1].tv_usec);
                                                                                printf("index = %d\n", ack_obtenu-window_head);


                    printf("timeout_finit = %d\n", t_finit.tv_usec);

                    t_ecart.tv_sec = t_finit.tv_sec - table_timeout[ack_obtenu - window_head].tv_sec;
                    
                    t_ecart.tv_usec = t_finit.tv_usec - table_timeout[ack_obtenu - window_head].tv_usec;
                    // if(window_tail == times){
                    //     timeout = old_timeout;
                    // }else{
                    timeout.tv_sec = (int)(0.7 * old_timeout.tv_sec + 0.3 * t_ecart.tv_sec);
                    timeout.tv_usec = (int)(0.7 * old_timeout.tv_usec + 0.3 * t_ecart.tv_usec);
                    // }
                    printf("timeout_ecart = %d\n\n", t_ecart.tv_usec);
                }
                else
                {
                    timeout = old_timeout;
